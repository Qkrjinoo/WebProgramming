...
function renderHistory(){
  const box=$('historyBox'), list=$('historyList');
  list.innerHTML='';
...
function showError(msg){
  ...
  if(!box){
    ...
    box.id='errorBox';box.className='error';
...
function updateUI(data){
  ...
  $('temp').innerText=`${data.main.temp.toFixed(1)}°C (체감 ${data.main.feels_like.toFixed(1)}°C)`;
  $('icon').src=`https://openweathermap.org/img/wn/${data.weather[0].icon}@2x.png`;
  $('weatherBox').classList.remove('hidden');

  const cond=data.weather[0].main;
  const m=weatherMap[cond]||{bg:'default-bg',pl:'37i9dQZF1DX1BzILRveYHb'};
  document.body.className=document.body.className.replace(/\b(\w+-bg)\b/g,'');
...
function initGeo(){
  if(!navigator.geolocation){
    $('searchForm').classList.remove('hidden');return;
  }
